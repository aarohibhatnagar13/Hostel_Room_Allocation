...
confirmRoomBooking: async (studentId: string, roomId: string, version: number) => {
  try {
    const res = await fetch(`${API_BASE_URL}/api/allocation/confirm`, 
...
runAllocation: async () => {
  try {
      const res = await fetch(`${API_BASE_URL}/api/allocation/run`, fetchOptions('POST'));
      return await handleResponse(res);
...
getAllocationHistory: async () => {
  try {
      const res = await fetch(`${API_BASE_URL}/api/allocation/history`, fetchOptions('GET'));
      return await handleResponse(res);
  } catch (e: any) { return { success: false, message: e.message }; }
},
...
getCachedRunDetails: async (runId: string) => {
  try {
      const res = await fetch(`${API_BASE_URL}/api/allocation/history/${runId}`, fetchOptions('GET'));
      return await handleResponse(res);
  } catch (e: any) { return { success: false, message: e.message }; }
}
};